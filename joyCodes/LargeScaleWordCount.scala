...
sc.textFile("/user/feed_weibo/warehouse/mds_datastrategy_feed_sample_filter_v2/dt=20170712/{001005_0.gz,001006_0.gz}").treeAggregate(scala.collection.mutable.Map.empty[String, Int])(seq, comb, 2).foreach(println)


val raw_data = sc.textFile("/user/feed_weibo/warehouse/mds_datastrategy_feed_sample_filter_v2/dt=20170712/{001005_0.gz,001006_0.gz}")
...
val raw_data = sc.textFile("/user/feed_weibo/warehouse/mds_datastrategy_feed_sample_filter_v2/dt=20170712")
// raw_data.count 1264762
// val littleData = raw_data.randomSplit(Array(0.1,0.9)).head
// val cached_data = raw_data.repartition(300).cache
// val splited_data =
val counted_data = raw_data.flatMap(_.split('\t')(121).split('|')).map(x => x -> 1L).reduceByKey(_ + _) 
counted_data.saveAsTextFile("/user/feed_weibo/enzhao/tag_count/mds_datastrategy_feed_sample_filter_v2/dt=20170712")
...
/user/feed_weibo/warehouse/mds_datastrategy_feed_sample_filter_v2/dt=20170712
/user/feed_weibo/enzhao/tag_count/mds_datastrategy_feed_sample_filter_v2/dt=20170712
spark-submit
spark-submit \
  --master yarn \
  --deploy-mode cluster \
